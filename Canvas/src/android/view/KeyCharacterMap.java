...
//    private static native KeyEvent[] nativeGetEvents(long ptr, char[] chars);
     ...
     * @param inputType
     */
    public KeyCharacterMap(int inputType){
        mPtr = inputType;
    }
    // Called from native
...
//        InputDevice inputDevice = im.getInputDevice(deviceId);
        ...
        if (inputDevice == null) { 
            throw new UnavailableException("Could not load key character map for device "
                + deviceId); 
         }
    ...
    public int get(int keyCode, int metaState) {
        ...
        char ch = (char)(keyCode);//nativeGetCharacter(mPtr, keyCode, metaState);
...
//        }
    ...
    public char getNumber(int keyCode) {
        ...
        return 0; //nativeGetNumber(mPtr, keyCode);
    }
...
//TODO - DEBUG it
        metaState = KeyEvent.normalizeMetaState(metaState);
        return (char)keyCode;//nativeGetMatch(mPtr, keyCode, chars, metaState);
    }
    ...
    public char getDisplayLabel(int keyCode) {
        //TODO debug it
        return  (char) keyCode; //nativeGetDisplayLabel(mPtr, keyCode);
    }
...
//        }
//
//        char displayLabel = (char) keyCode;//nativeGetDisplayLabel(mPtr, keyCode);
//        if (displayLabel == 0) {
//            return false;
//        }
//
...
//                KeyEvent.META_ALT_ON | KeyEvent.META_SHIFT_ON);
    ...
    public boolean isPrintingKey(int keyCode) {
        int type = keyCode; 
                //Character.getType(nativeGetDisplayLabel(mPtr, keyCode));
    ...
    public int getKeyboardType() {
        if(mPtr == InputDevice.GLES_KEYBOARD){
            return InputDevice.KEYBOARD_TYPE_ALPHABETIC;
        }
        return InputDevice.KEYBOARD_TYPE_NON_ALPHABETIC ;//nativeGetKeyboardType(mPtr);
    }
    ...
    public static boolean[] deviceHasKeys(int[] keyCodes) {
        ...
        boolean[] hasIt = new boolean[keyCodes.length];
