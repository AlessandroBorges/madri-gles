...
public class Bitmap {
         ...
         /**
            ...
            private  Object /*NinePatch.InsetStruct*/ mNinePatchInsets; // may be null
            private int mWidth;
            private int mHeight;
    ...
    static int getDefaultDensity() {
        ...
        // using 100 for Desktop Screen.
        sDefaultDensity = 96; // DisplayMetrics.DENSITY_DEVICE;
    ...
    Bitmap(long nativeBitmap,
            ...
            byte[] ninePatchChunk,
            Object /* NinePatch.InsetStruct */ninePatchInsets) {
        ...
        mFinalizer = new BitmapFinalizer(this, nativeAllocationByteCount);
            ...
            public int getGenerationId() {
                ...
                return (int)nativeGenerationId(this);
            }
            ...
            public void copyPixelsToBuffer(Buffer dst) {
                ...
                long elements = dst.remaining();
                long bufferSize = elements << shift; // in bytes
                ...
                //nativeCopyPixelsToBuffer(dst);
                
                if(shift==0){ // ByteBuffer case
                    ByteBuffer dstBuffer = (ByteBuffer) dst;
                    int len = mBuffer.length;
                    for (int i = 0; i < len; i++) {
                        dstBuffer.put(mBuffer[i]);
                   }
                } else if(shift==1){ // ShortBuffer Case                    
                    ShortBuffer dstBuffer = (ShortBuffer) dst;
                    int len = mBuffer.length; // half size
                    short color=0;
                    for (int i = 0; i < len;) {
                        color  = (short)((mBuffer[i++] << 8) & 0xFF00);
                        color |= (short)((mBuffer[i++]     ) & 0x00ff); 
                        dstBuffer.put(color);
                   }
                }else{ // IntBufferCase
                    IntBuffer dstBuffer = (IntBuffer) dst;
                    int len = mBuffer.length; // quarter size
                    int color=0;
                    for (int i = 0; i < len;) {
                        color  = (mBuffer[i++] << 24) & 0xFF000000; // alpha
                        color |= (mBuffer[i++] << 16) & 0x00FF0000;// red
                        color |= (mBuffer[i++] <<  8) & 0x0000FF00;// red
                        color |= (mBuffer[i++]      ) & 0x000000FF;// red
            ...
            public void copyPixelsFromBuffer(Buffer src) {
                ...
                //nativeCopyPixelsFromBuffer(this, src);
                
                if(shift==0){ // ByteBuffer case
                    ByteBuffer srcBuffer = (ByteBuffer) src;
                    int len = mBuffer.length;
                    for (int i = 0; i < len; i++) {
                        mBuffer[i] = srcBuffer.get();
                   }
                } else if(shift==1){ // ShortBuffer Case                    
                    ...
                    for (int i = 0; i < len;) {                        
                        color = srcBuffer.get();
                        mBuffer[i++]  = (byte)((color >> 8) & 0xFF);
                        mBuffer[i++]  = (byte)((color     ) & 0xFF);
                   }
                }else{ // IntBufferCase
                    ...
                    for (int i = 0; i < len;) {
                        color = srcBuffer.get();                    
                        mBuffer[i++] = (byte)((color >> 24) & 0xFF); // alpha
                        mBuffer[i++] = (byte)((color >> 16) & 0xFF);// red
                        mBuffer[i++] = (byte)((color >>  8) & 0xFF);// green
                        mBuffer[i++] = (byte)((color      ) & 0xFF);// blue
                   }
                }
                                       ...
                                       boolean filter) 
    {
             ...
             * @throws IllegalArgumentException if the width or height are <= 0
             */
            private static Bitmap createBitmap( DisplayMetrics display, 
                                                int width, int height,
...
//    }
       ...
       // Trace.traceEnd(Trace.TRACE_TAG_RESOURCES);
            ...
            public final int getRowBytes() {
                ...
                if(mConfig==Config.ALPHA_8) return mWidth;
                if(mConfig==Config.RGB_565) return mWidth * 2;
                if(mConfig==Config.ARGB_8888 || mConfig==Config.ARGB_4444) return mWidth * 4;
                return 0;
...
//    }
            ...
            private int mStride;
            private static int nativeBitmapId = 0;

    /**
     * Codes from
     *  https://android.googlesource.com/platform/frameworks/base/+/android-5.1.1_r1/core/jni/android/graphics/Bitmap.cpp
     *  and 
     *  https://android.googlesource.com/platform/frameworks/base/+/android-5.1.1_r1/core/jni/android/graphics/Graphics.cpp
     *  
     * @param colors
     * @param offset
     * @param stride
                                                  ...
                                                  boolean mutable)
    {
       ...
       // ARGB_4444 is a deprecated format, convert automatically to 8888
        if (nativeConfig == Config.ARGB_4444.nativeInt) {
            nativeConfig = Config.ARGB_8888.nativeInt;
            }
        ...
        Bitmap bitmap = new Bitmap( ++nativeBitmapId, 
                                    buffer, width, height, 
                                    0, 
                                    mutable, 
        ...
        bitmap.nativeSetPixels(colors, offset, stride, 0, 0, width, height);
        return bitmap;
    }
    ...
    private static boolean nativeCompress(Bitmap bm, int format,
        ...
        return true;
             ...
             * @param color
             */
           // private static void nativeSetPixel(long nativeBitmap, int x, int y, int color){
            private void nativeSetPixel(int x, int y, int color){
                int pos = x + y * mWidth;
             ...
             * 
             * 
             * @param colors ARGB colors to set
             * @param offset   The index of the first color to read from pixels[]
             * @param stride   The number of colors in pixels[] to skip between rows.
             *                 Normally this value will be the same as the width of
             *                 the bitmap, but it can be larger (or negative).
            ...
            {                
                ...
                for(int dstY = y, line=0; (dstY < height) && (dstY < maxY); dstY++, line++){
                    int srcPos = offset + line * stride; 
                    
                    for(int dstX = x; (dstX < width) && (dstX < maxX); dstX++){                        
                      nativeSetPixel(dstX, dstY, colors[srcPos++]);
                    }
                }
            }
            
            
            //private static void nativeCopyPixelsToBuffer(long nativeBitmap, Buffer dst);
//            private void nativeCopyPixelsToBuffer(/*Bitmap bitmap,*/ Buffer dst){ 
//                // no op
//            }
            
            //private static void nativeCopyPixelsFromBuffer(long nativeBitmap, Buffer src);            
//            private static void nativeCopyPixelsFromBuffer(Buffer src){
//              //  no op
//            }
            
            private static long nativeGenerationId(Bitmap bitmap){
                return bitmap.mNativeBitmap;
            }
             ...
             * @return
             */
            private static char RGB888to565(int p24){
                p24 = p24 & 0x00FFffFF; // cut alpha off
                int b = (p24 >> 3) & 0x00001F;
                int g = (p24 >> 5) & 0x0007E0;
                int r = (p24 >> 8) & 0x00f800;                
                return (char) (r | g | b);        
            }
            
            
            /**
             * to be used by RGB888to565 only;
             * @see #RGB888to565(int, byte[])
             */
