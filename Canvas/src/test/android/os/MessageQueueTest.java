    ...
    public void testFieldIntegrity() throws Exception {
        ...
        TestHandlerThread tester = new TestFieldIntegrityHandler() {
            ...
            public void handleMessage(Message msg) {
                super.handleMessage(msg);
                if (msg.what == 0) {
                    msg.flags = Message.FLAG_IN_USE;//-1; TODO check this
                    msg.what = 1;
                    msg.arg1 = 456;
                    msg.arg2 = 789;
                    msg.obj = this;
                    ...
                    if (newMsg.getWhen() != 0) {
                        failure(new RuntimeException(String.format(
                                "newMsg.when is %d should be 0 after copyFrom", newMsg.getWhen())));
                    }
                    if (newMsg.getTarget() != mHandler) {
                        failure(new RuntimeException(
                                "newMsg.target is NOT mHandler after copyFrom"));
                    }
                    if (newMsg.getCallback() != null) {
                        failure(new RuntimeException(
